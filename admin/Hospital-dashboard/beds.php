...
// Only allow hospitals to access this page
...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    if (isset($_POST['add_bed'])) {
        ...
        if(!$stmt->execute()){
            die("Add error: " . $stmt->error);
        }
    } elseif (isset($_POST['update_bed'])) {
        $bed_id = $_POST['bed_id'];
        $bed_type = $_POST['bed_type'];
        $bed_count = $_POST['bed_count'];
        $stmt = $conn->prepare("UPDATE beds SET bed_type=?, bed_count=? WHERE bed_id=? AND hospital_id=?");
        $stmt->bind_param("siii", $bed_type, $bed_count, $bed_id, $hospital_id);
        if(!$stmt->execute()){
...
<head>
    ...
    <style>
        ...
        .vaccinated { background-color: rgba(40, 167, 70, 0.84); color: white; }
        .pending { background-color: rgba(255, 234, 7, 0.8); color: black; }
        table { width: 90%; border-collapse: collapse; margin: 20px 0; }
        th, td { padding: 12px; text-align: left; border-bottom: 1px solid #ddd; }
        th { background-color: rgba(88, 80, 207, 0.669); color: white; }
...
<body>
        ...
        <div class="container">
            ...
            <div class="modal-overlay" id="bedModal">
                <div class="modal-content">
                    <form id="bedForm" method="POST" action="beds.php">
    ...
    <script>
        ...
        addBedBtn.addEventListener('click', () => {
            ...
            bedTypeInput.value = '';
            bedCountInput.value = '';
            // Remove any existing hidden inputs for action determination
            document.querySelectorAll('#bedForm input[type="hidden"][name="add_bed"], #bedForm input[type="hidden"][name="update_bed"]').forEach(el => el.remove());
        ...
        document.querySelectorAll('.editBtn').forEach(button => {
            button.addEventListener('click', () => {
                ...
                // Remove any existing hidden inputs then add update action input
                document.querySelectorAll('#bedForm input[type="hidden"][name="add_bed"], #bedForm input[type="hidden"][name="update_bed"]').forEach(el => el.remove());
